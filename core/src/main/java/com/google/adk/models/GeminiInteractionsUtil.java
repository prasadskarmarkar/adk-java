 ...
 */
public final class GeminiInteractionsUtil {

  private static final Logger logger = LoggerFactory.getLogger(GeminiInteractionsUtil.class);

  private GeminiInteractionsUtil() {}
  ...
  public static Flowable<LlmResponse> generateContentViaInteractions(
    ...
    logger.debug(
        "Interactions API request: model={}, previousInteractionId={}",
        modelName,
        llmRequest.previousInteractionId().orElse("<none>"));
  ...
  private static Flowable<LlmResponse> nonStreamingCreate(
    ...
    return Flowable.fromFuture(future)
        .map(
            interaction -> {
              logger.debug(
                  "Interactions API response: id={}, status={}",
                  interaction.id(),
                  interaction.status());
  ...
  static LlmResponse convertInteractionToLlmResponse(Interaction interaction) {
    ...
    for (Content output : outputs) {
      convertInteractionsContentToPart(output).ifPresent(parts::add);
    }

    if (!parts.isEmpty()) {
      builder.content(com.google.genai.types.Content.builder().role("model").parts(parts).build());
    }
  ...
  private static Optional<LlmResponse> convertContentDeltaToLlmResponse(ContentDelta contentDelta) {
    ...
    if (delta instanceof TextDelta textDelta) {
      ...
      if (name != null || args != null) {
        ...
        com.google.genai.types.Part part =
            com.google.genai.types.Part.builder().functionCall(fcBuilder.build()).build();

        return Optional.of(
            LlmResponse.builder()
                .content(com.google.genai.types.Content.builder().role("model").parts(part).build())
                .partial(true)
                .build());
      }
      return Optional.empty();
  ...
  static Optional<com.google.genai.types.Part> convertInteractionsContentToPart(Content content) {
    if (content instanceof TextContent textContent) {
      ...
      String text = !thoughtText.isEmpty() ? thoughtText.toString() : "";
            ...
            configTools -> {
              for (com.google.genai.types.Tool configTool : configTools) {
                ...
                if (configTool.codeExecution().isPresent()) {
                  if (result.stream().noneMatch(t -> t instanceof CodeExecution)) {
                    result.add(CodeExecution.builder().build());
                  }
                }
                if (configTool.urlContext().isPresent()) {
                  if (result.stream().noneMatch(t -> t instanceof UrlContext)) {
                    result.add(UrlContext.builder().build());
                  }
                }
                if (configTool.computerUse().isPresent()) {
                  if (result.stream().noneMatch(t -> t instanceof ComputerUse)) {
