...
public class SpannerDialect implements SqlDialect {
  ...
  /**
   * Cloud Spanner does not support SELECT ... FOR UPDATE (pessimistic locking). Concurrent
   * appendEvent calls on the same session may result in lost state updates. Spanner's read-write
   * transactions provide serializable isolation, but without row-level locking the
   * read-modify-write pattern in persistEventToDatabase is not fully protected against concurrent
   * modifications.
   */
  @Override
