...
/**
 * JDBC-based implementation of {@link BaseSessionService} with zero external persistence
 * dependencies.
 *
 * <p>This service provides persistent session management using pure JDBC and HikariCP connection
 ...
 *
 * <ul>
 *   <li>Pure JDBC implementation with HikariCP connection pooling
...
public class DatabaseSessionService implements BaseSessionService, AutoCloseable {
            ...
            () -> {
              return jdbcTemplate.inTransaction(
                  ops -> {
                    ...
                    Optional<Integer> limit = config.flatMap(GetSessionConfig::numRecentEvents);
  ...
  public Single<Event> appendEvent(Session session, Event event) {
    ...
    return BaseSessionService.super
        ...
        .flatMap(
            updatedEvent ->
                persistEventToDatabase(session.appName(), session.userId(), session.id(), event)
                    .andThen(Single.just(updatedEvent)));
  }
