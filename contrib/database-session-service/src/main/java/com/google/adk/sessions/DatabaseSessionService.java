...
public class DatabaseSessionService implements BaseSessionService, AutoCloseable {
  ...
  public DatabaseSessionService(String jdbcUrl, Map<String, Object> properties) {
    ...
    runMigrations(dataSource, jdbcUrl);
  ...
  private void runMigrations(javax.sql.DataSource dataSource, String jdbcUrl) {
    try {
   ...
   *
   * <p>The sessions are returned without events but with merged state from all three tiers (app,
   * user, session). Use {@link #getSession} to retrieve full session details including events.
   *
   * <p>Sessions are ordered by update_time descending (most recently updated first).
   *
   * @param appName the application name (must not be null)
   * @param userId the user identifier (must not be null)
                    ...
                    List<Session> sessions =
                        sessionRows.stream()
                            .map(
                                row ->
                                    toDomainSession(
                                        row, Collections.emptyList(), appState, userState))
   ...
   *
   * @param appName the application name (must not be null)
   * @return a Maybe that emits the app state map if found, or completes empty if no app state
   *     exists
   */
  public Maybe<Map<String, Object>> getAppState(String appName) {
    return Maybe.fromCallable(
            () -> {
              return jdbcTemplate.inTransaction(
                  ops -> {
   ...
   * @param appName the application name (must not be null)
   * @param userId the user identifier (must not be null)
   * @return a Maybe that emits the user state map if found, or completes empty if no user state
   *     exists
   */
  public Maybe<Map<String, Object>> getUserState(String appName, String userId) {
    return Maybe.fromCallable(
            () -> {
              return jdbcTemplate.inTransaction(
                  ops -> {
