...
public class RunLoopDemo {

  private static final String APP_NAME = "loop_quality_refinement_app";
  private static final String USER_ID = "quality_user";
  ...
  private static String getDatabaseUrl() {
    ...
    String h2File = System.getProperty("user.dir") + "/quality_refinement_db";
  ...
  public static void main(String[] args) {
    System.out.println("╔════════════════════════════════════════════════════════════╗");
    System.out.println("║     LOOP AGENT TUTORIAL: Data Quality Refinement          ║");
    ...
    try (DatabaseSessionService sessionService = new DatabaseSessionService(dbUrl)) {
      ...
      Runner runner =
          new Runner(
              DataQualityRefinementAgent.QUALITY_REFINEMENT_LOOP,
              APP_NAME,
              new InMemoryArtifactService(),
              sessionService,
              new InMemoryMemoryService());
      System.out.println("✓ Runner initialized with LoopAgent (max 5 iterations)");
      System.out.println();
  ...
  private static void runAnalysis(Runner runner, String[] datasets) {
    ...
    for (int i = 0; i < datasets.length; i++) {
      ...
      Content userContent =
          Content.builder()
              .role("user")
              .parts(List.of(Part.fromText("Assess the quality of this dataset: " + dataset)))
              .build();
      ...
      for (Event event :
        ...
        if (event.author().equals("data_quality_refinement_agent") && event.content().isPresent()) {
          iterationCount++;
    ...
    System.out.println("╔════════════════════════════════════════════════════════════╗");
    System.out.println("║  Loop refinement completed!                                ║");
    System.out.println("║  Session ID: " + sessionId + "  ║");
    System.out.println("╚════════════════════════════════════════════════════════════╝");
    System.out.println();
    System.out.println("💾 Session has been persisted to the database!");
    System.out.println("   - All loop iterations saved");
    System.out.println("   - ExitLoopTool usage tracked");
