...
Run the Loop Agent tutorial with DatabaseSessionService.
...
SELECT 
  ...
  JSON_UNQUOTE(JSON_EXTRACT(event_data, '$.content.parts[0].text')) as message
FROM events 
WHERE session_id = (SELECT id FROM sessions ORDER BY create_time DESC LIMIT 1)
ORDER BY timestamp;" 2>/dev/null
    
    echo ""
    echo "📋 Events Data (JSON Format):"
    docker exec mysql-adk mysql -u"$DB_USER" -padk_password -D"$DB_NAME" -N -e "
SELECT JSON_ARRAYAGG(event_data ORDER BY timestamp)
FROM events 
WHERE session_id = (SELECT id FROM sessions ORDER BY create_time DESC LIMIT 1);" 2>/dev/null | jq '.' 2>/dev/null || echo "   Unable to format JSON output"
  fi
